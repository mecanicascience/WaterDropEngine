...
function receivedMessage(message: Message, socket: Socket) {
    // Check identifier and title
    if (message.identifier == 0 && message.title == 0) {
        // Shared memory config
        // Read payload as string
        const path = message.payload.toString('utf8', 0, message.payload.length);
...
export function createIPCServer() {
    ...
    ipc.serve(
        ...
        function () {
            ...
            ipc.server.on(
                'socket.disconnected',
                function (_socket: Socket, _destroyedSocketID: string) {
                    console.log('Client has disconnected from editor.');
